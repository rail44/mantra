...
func Add(a, b int) int {
    ...
    assert!(result.contains("mantra:checksum:"), "Checksum should be added");
    assert!(result.contains("return a + b"), "Generated code should be present");
...
async fn test_document_manager_actor_commands() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let actor_handle = tokio::spawn(async move {
        doc_manager.run_actor(rx).await
    });
    
    // Test GetSource command
    let (response_tx, response_rx) = oneshot::channel();
    tx.send(DocumentCommand::GetSource { response: response_tx }).await?;
